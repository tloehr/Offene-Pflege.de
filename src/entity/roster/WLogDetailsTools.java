...
public class WLogDetailsTools {
    ...
    public static final int OTHER = 5; // For sick or holiday hours
    public static final int ADDITIONAL = 6;

    public static final String[] TYPES = new String[]{"Tag1", "Nacht1", "Nacht2", "Tag2", "Pause", "Andere", "Zusätzlich"};
...
//        Rplan myRplan = em.merge(rplan);
        ...
        if (symbol.getCalc() == Symbol.PVALUE) return;

        if (symbol.getCalc() == Symbol.XWERT) {
            myRplan.getWLogDetails().add(new WLogDetails(BigDecimal.ZERO, BigDecimal.ZERO, OTHER, myRplan));
        } else if (symbol.getCalc() == Symbol.KWERT) {
            myRplan.getWLogDetails().add(new WLogDetails(parameterSet.getDayValue(), BigDecimal.ZERO, OTHER, myRplan));
        } else if (symbol.getCalc() == Symbol.UWERT) {
            myRplan.getWLogDetails().add(new WLogDetails(parameterSet.getDayValue(), BigDecimal.ZERO, OTHER, myRplan));
        } else { // AWert
