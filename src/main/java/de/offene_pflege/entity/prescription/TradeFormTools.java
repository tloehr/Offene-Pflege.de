...
public class TradeFormTools {
     ...
     * <code>getSuitableInventoriesForThisTradeForm</code>. Sie implementiert Punkt 1 der dort beschriebenen 2
     * Antworten.
     *
     * @param bewohner
     * @param tradeform
     * @return Wenn die Darreichung zu einem früheren Zeitpunkt schonmal zugeordnet war, dann wird dieser Vorrat zurück
     * gegeben. Ansonsten <code>null</code>
     */
    ...
    public static List<TradeForm> get_equivalent_tradeforms_according_to_inventories(EntityManager em, Resident resident, TradeForm tradeform) {
        Query query = em.createQuery(" SELECT DISTINCT stock.tradeform FROM MedInventory inv " +
    ...
    public static List<TradeForm> get_tradeforms_in_this_inventory(EntityManager em, MedInventory medInventory) {
        Query query = em.createQuery(" SELECT DISTINCT stock.tradeform FROM MedInventory inv " +
                " JOIN inv.medStocks stock " +
                " WHERE inv = :inventory");
        query.setParameter("inventory", medInventory);
    ...
    /**
     * Um diese Methode zu verstehen muss man sich einige Fakten und Konzepte klar machen, die wir bei der Entwicklung
     * von OPDE bzgl. der Medikamente angewendet haben. Bitte beachten Sie dass mit dieser Methode der Punkt 2 der
     * beschrieben Antworten implementiert wird. Punkt 1 finden sie in der Methode <code>getInventory4TradeForm</code>
     * umgesetzt.
     * <p/>
     * Eine <b>Darreichung</b> ist eine genaue Definition eines Medikaments, bei einer bestimmten Stärke, ein
     * Hersteller, eine Darreichungsform (bitte vom Begriff Darreichung unterscheiden), eine Anwendungseinheit, eine
     * Packungseinheit. usw. Das heisst, bei einer Verordnung wird immer eine Darreichung verordnet. Diese ist
     * ausreichend, ja viel genauer als eine Medikamentenangabe.
     * <p/>
     * Über die Zeit kann es immer wieder vorkommen, dass eine Verordnung zwar bestehen bleibt, aber das Medikament sich
     * verändert. Allerdings nicht beliebig sondern <b>wirkstoffgleich</b>. Das passiert z.B. wenn zu Beginn, sagen wir
     * Aspirin von Bayer verordnet wurde. Nach einigen Wochen schreibt der GP dann aber statt dessen ASS 100 von
     * ratiopharm auf. Diese Medikament kommt dann an und passt aber nicht zu der ursprünglichen Verordnung (aus Sicht
     * von OPDE). Es ist ja eine andere <b>Darreichung</b>.
     * </p>
     * Trotzdem muss dieses neue Präparat ebenfalls mit auf den Vorrat, der für die Verordnung verwendet wird,
     * draufgebucht werden. Das einzige auf dem OPDE hier besteht ist, dass die beiden Präparate dieselbe bzw. eine
     * äquivalente PrinterForm haben.So können mit der Zeit die Vorräte Bestände mit ganz unterschiedlichen
     * Darreichungen besessen haben.
     * <p>
     ...
     * <p>
     * Natürlich sind alle nachfolgenden Überlegungen <i>bewohnerbezogen</i>. Ein Vorrat gehört immer <b>genau einem</b>
     * Bewohner.
     * <p>
     ...
     * <li>Trifft Punkt 1 nicht zu, dann suchen wir alle Vorräte, die Darreichungen enthalten mit einer passenden DosageForm. Passend heisst hier entweder dieselbe PrinterForm oder eine
     * äquivalente DosageForm (z.B. Tabletten sind zu Dragees gleichwertig wie zu Filmtabletten etc.).</li>
     * </ol>
     *
     * @param resident
     * @param tradeform
    ...
    /**
     * returns a pair of integers. the first one stands for the days. if there are more than 7 days then the second
     * integer is used as the weeks figure. the days are set to 0 then.
     *
     * @param tradeForm
     * @return
     */
