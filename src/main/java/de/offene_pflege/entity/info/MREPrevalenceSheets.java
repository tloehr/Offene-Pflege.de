...
//        LocalTime morning8 = new LocalTime(8, 0); // eight o'clock
        ...
        for (Resident resident : listResidents) {
            for (ResInfo resInfo : ResInfoTools.getAll(resident, getResInfoTypeByType(ResInfoTypeTools.TYPE_ROOM), targetDate.toDateTimeAtStartOfDay().toDate(), SYSCalendar.eod(targetDate).toDate())) {
    ...
    private void fillALineInSheet2(Prescription prescription, int runningRow) {
        ...
        Properties properties = resInfo != null ? ResInfoTools.getContent(resInfo) : new Properties();
    ...
    private ArrayList<Prescription> fillALineInSheet1(Resident resident) {
        ...
        for (ResInfo resInfo : listHospital) {
            Properties p = ResInfoTools.getContent(resInfo);
