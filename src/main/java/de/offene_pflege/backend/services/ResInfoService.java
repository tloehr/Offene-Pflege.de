...
public class ResInfoService implements HasLogger {
    ...
    public static boolean isCurrentlyValid(ResInfo resInfo) {
        Date now = new Date();
        return resInfo.getFrom().compareTo(now) <= 0 && resInfo.getTo().compareTo(now) >= 0;
    }

    public static String getPITAsHTML(ResInfo resInfo) {
        String result = "";
        DateFormat df = isSingleIncident(resInfo) || isBySecond(resInfo) ? DateFormat.getDateTimeInstance(DateFormat.DEFAULT, DateFormat.SHORT) : DateFormat.getDateInstance();

        if (isSingleIncident(resInfo)) {
            result += df.format(resInfo.getFrom()) +
                    "<br/>" +
                    resInfo.getUserON().getFullname();

        } else if (isClosed(resInfo)) {

            result += "<table id=\"fonttext\" border=\"0\" cellspacing=\"0\">";
            result += "<tr>";
            result += "<td valign=\"top\">" + df.format(resInfo.getFrom()) + "</td>";
            result += "<td valign=\"top\">&raquo;</td>";
            result += "<td valign=\"top\">" + df.format(resInfo.getTo()) + "</td>";
            result += "</tr>\n";
            result += "<tr>";
            result += "<td valign=\"top\">" + resInfo.getUserON().getFullname() + "</td>";
            result += "<td valign=\"top\">&raquo;</td>";
            result += "<td valign=\"top\">" + resInfo.getUserOFF().getFullname() + "</td>";
            result += "</tr>\n";
            result += "</table>\n";

        } else {
            result += df.format(resInfo.getFrom()) + "&nbsp;&raquo;&raquo;" +
                    "<br/>" +
                    resInfo.getUserON().getFullname();
        }
    ...
    public static boolean isClosed(ResInfo resInfo) {
        return resInfo.getResInfoType().getIntervalMode() != ResInfoTypeTools.MODE_INTERVAL_SINGLE_INCIDENTS && resInfo.getTo().before(new Date());
    }

    public static boolean isBySecond(ResInfo resInfo) {
        return resInfo.getResInfoType().getIntervalMode() == ResInfoTypeTools.MODE_INTERVAL_BYSECOND;
    }

    public static boolean isSingleIncident(ResInfo resInfo) {
        return resInfo.getResInfoType().getIntervalMode() == ResInfoTypeTools.MODE_INTERVAL_SINGLE_INCIDENTS;
    }

    public static boolean isNoConstraints(ResInfo resInfo) {
        return resInfo.getResInfoType().getIntervalMode() == ResInfoTypeTools.MODE_INTERVAL_NOCONSTRAINTS;
    }
...
//        min = firstHauf.getFrom();
    ...
    public static boolean isGone(Resident resident) {
        ResInfo bwinfo_hauf = ResInfoService.getLastResinfo(resident, ResInfoTypeTools.getByType(ResInfoTypeTools.TYPE_STAY));
    ...
    public static boolean isKZP(Resident resident) {
        return isKZP(ResInfoService.getLastResinfo(resident, ResInfoTypeTools.getByType(ResInfoTypeTools.TYPE_STAY)));
    }
...
//            if (!resInfo.getResInfoType().isDeprecated()) set.add(resInfo.getResInfoType());
    ...
    public static boolean wasAway(Resident resident, LocalDate targetDate) {
        ArrayList<ResInfo> listAbsence = ResInfoService.getAll(resident, ResInfoTypeTools.getByType(ResInfoTypeTools.TYPE_ABSENCE), targetDate, targetDate);
    ...
    public static String getContentAsPlainText(ResInfo resInfo) {
        return ResInfoService.parseResInfo(resInfo).render(new ResInfoTextRenderer());
    }
...
//    }
    ...
    public static String getTXReportHeader(Resident resident, boolean withlongheader) {
        ...
        for (ResInfo resInfo : ResInfoService.getAll(resident, ResInfoTypeTools.getByType(ResInfoTypeTools.TYPE_ROOM), SYSCalendar.midOfDay().toDate(), SYSCalendar.midOfDay().toDate())) {
...
//            theoreticalweight = BigDecimal.ZERO;
        ...
        ResInfo bwinfo_pstf = ResInfoService.getLastResinfo(resident, ResInfoTypeTools.TYPE_NURSING_INSURANCE);
        if (bwinfo_pstf != null) {
        ...
        Date absentSince = ResInfoService.absentSince(resident);
        if (absentSince != null) {
        ...
        ResInfo bwinfo_angeh = ResInfoService.getLastResinfo(resident, ResInfoTypeTools.getByType(ResInfoTypeTools.TYPE_SOZIALES));
        ...
        final ArrayList<ResInfo> specialists = ResInfoService.getActive(resident, ResInfoTypeTools.getByType(ResInfoTypeTools.TYPE_SPECIALIST));
...
//            result += "<br/><br/>";
    ...
    public static String getFallsAnonymous(int monthsback, Closure progress) {
        ...
        Station exResident = StationService.create(SYSTools.xx("opde.controlling.exResidents"), null);
    ...
    public static void closeAll(EntityManager em, Resident resident, Date enddate, String reason) throws Exception {
        ...
        for (ResInfo info : bwinfos) {
            ...
            if (info.getResInfoType().getType() == ResInfoTypeTools.TYPE_STAY) {
                Properties props = ResInfoService.getContent(info);
                props.setProperty(ResInfoTypeTools.STAY_KEY, reason);
                ResInfoService.setContent(info, props);
            }
        }
    }
