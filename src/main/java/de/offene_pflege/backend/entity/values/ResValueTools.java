...
//        query.setParameter(3, resident.getId());
...
//    }
    ...
    public static Optional<ResValue> getMostRecentBefore(Resident resident, short type, LocalDateTime before) {
        ...
        query.setParameter("type", type);
        query.setParameter("before", JavaTimeConverter.toDate(before));

        Optional<ResValue> resvalue;
        try {
            resvalue = Optional.of((ResValue) query.getSingleResult());
        } catch (Exception e) {
            resvalue = Optional.empty();
        } finally {
            em.close();
        }
...
//    }
    ...
    public static ArrayList<ResValue> getResValuesNoEdits(Resident resident, short type, java.time.LocalDate f, java.time.LocalDate t) {

        //        DateTime theYear = new DateTime(year, 1, 1, 0, 0, 0);
//        javaDateTime from = f.toDateTimeAtStartOfDay();
//        DateTime to = SYSCalendar.eod(t);
        ...
        query.setParameter("resident", resident);
        query.setParameter("type", type);
        query.setParameter("from", JavaTimeConverter.toDate(f.atStartOfDay()));
        query.setParameter("to", JavaTimeConverter.toDate(t.atTime(23, 59, 59)));
        ArrayList<ResValue> list = new ArrayList<ResValue>(query.getResultList());
        em.close();
...
//        NumberFormat dcf = DecimalFormat.getNumberInstance();
    ...
    /**
     * erstellt eine kombinierte gewichts und bmi statistik in html. ein bmi wert kann nur ermittelt werden, wenn es
     * eine größenangabe für den betroffenen BW gibt.
     *
     * @param monthsback die Anzahl von Monaten (vom aktuellen Zeitpunkt aus gerechnet) die betrachtet werden sollen.
     * @param retiredToo gibt an, ob auch ehemalige BWs mit in die Liste aufgenommen werden sollen. Ehemalig wird nur
     *                   vom aktuellen Zeitpunkt aus berücksichtigt. Also ob jemand **jetzt** noch in der Einrichtung
     *                   ist. Nicht ob er zum Zeitpunkt der Datenerhebung bei uns war.
    ...
    public static String getWeightStats(int monthsback, boolean retiredToo, Closure progress) {
        ...
        for (Resident resident : listResidents) {
            ...
            BigDecimal firstBMI = height.isPresent() ? firstValue.getVal1().divide(height.get().getVal1().pow(2), 2, RoundingMode.HALF_UP) : null;
     ...
     * ermittelbar war. Z.B. weil es keine Werte gibt.
     */
...
//        DateTime now = new DateTime();
        ...
        for (Resident resident : listResident) {
            ...
            if (!ResInfoService.isAway(resident)) {  // https://github.com/tloehr/Offene-Pflege.de/issues/97
...
//    }
    ...
    public static HashMap<java.time.LocalDate, MutableTriple<BigDecimal, BigDecimal, BigDecimal>> getLiquidBalances(Resident resident, java.time.LocalDate before, java.time.LocalDate after) {
...
//        ArrayList<Object[]> result = new ArrayList(query.getResultList());
        ...
        final HashMap<java.time.LocalDate, MutableTriple<BigDecimal, BigDecimal, BigDecimal>> resultMap = new HashMap<>();

        trinkQuery.getResultList().forEach(o -> {
            ResValue resValue = (ResValue) o;
            java.time.LocalDate localDate = JavaTimeConverter.toJavaLocalDateTime(resValue.getPit()).toLocalDate();
            resultMap.putIfAbsent(localDate, new MutableTriple<>(BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO));

            BigDecimal in = resultMap.get(localDate).getLeft();
            BigDecimal out = resultMap.get(localDate).getMiddle();
            BigDecimal sum = resultMap.get(localDate).getRight();

            if (resValue.getVal1().compareTo(BigDecimal.ZERO) > 0) in = in.add(resValue.getVal1());
            if (resValue.getVal1().compareTo(BigDecimal.ZERO) < 0) out = out.add(resValue.getVal1());
            sum = sum.add(resValue.getVal1());

            resultMap.get(localDate).setLeft(in);
            resultMap.get(localDate).setMiddle(out);
            resultMap.get(localDate).setRight(sum);
