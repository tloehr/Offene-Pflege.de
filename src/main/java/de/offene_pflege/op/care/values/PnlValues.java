 ...
 * the JPanel with the content for this type. that are the years for which there are stored values. empty if no values
 * are present createCP4Year - Creates the year CP createContent4Year - creates the JPanel for this year (if the type is
 * LIQUID Balance it is further divided by weeks. otherwise the values themselfs are shown here createCP4Week - creates
 * the week CPs for the liquid balance values createContent4Week - create the JPanel for the week (LIQUID BALANCE)
...
//        GUITools.addAllComponents(mypanel, addKey());
    ...
    private CollapsiblePane createCP4Type(final Resvaluetypes vtype) {
        ...
        cptitle.getButton().setForeground(ResvaluetypesTools.getColor(vtype));
        ...
        if (OPDE.getAppInfo().isAllowedTo(InternalClassACL.UPDATE, internalClassID)) {
            ...
            btnAdd.setEnabled(ResidentTools.isActive(resident));
        }
...
//        cpType.setBackground(getColor(vtype, SYSConst.medium1));
    ...
    private CollapsiblePane createCP4Year(final Resvaluetypes vtype, final int year) {
        ...
        cptitle.getButton().setForeground(GUITools.blend(ResvaluetypesTools.getColor(vtype), Color.BLACK, 0.75f));
        ...
        if (OPDE.getAppInfo().isAllowedTo(InternalClassACL.PRINT, internalClassID)) {
            ...
            btnPrintYear.addActionListener(actionEvent -> {
                ...
                SYSFilesService.print(SYSTools.toHTML(ResValueTools.getAsHTML(mapType2Values.get(keyYears))), true);
            });
...
//            }
    ...
    private JPanel createContentPanel4Year(final Resvaluetypes vtype, final int year) {
        ...
        for (final ResValue resValue : myValues) {
            ...
            pnlTitle.getMain().setBackground(GUITools.blend(ResvaluetypesTools.getColor(vtype), Color.WHITE, 0.1f));
            ...
            if (!resValue.getAttachedProcessConnections().isEmpty()) {
                ...
                btnProcess.setEnabled(resValue.isActive() && OPDE.getAppInfo().isAllowedTo(InternalClassACL.UPDATE, internalClassID));
    ...
    private JPanel getMenu(final ResValue resValue) {
        ...
        boolean doesNotBelongToResInfos = ResInfoService.getInfosFor(resValue).isEmpty();
