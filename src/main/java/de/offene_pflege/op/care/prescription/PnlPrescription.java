...
public class PnlPrescription extends NursingRecordsPanel {
    ...
    public void switchResident(Resident res) {
        ...
        prepareSearchArea();
        log.debug("from switchResident");
        reloadDisplay();
    }

    @Override
    public void reload() {
        log.debug("from reload");
        reloadDisplay();
    }
...
//        }
    ...
    private List<Component> addCommands() {
        ...
        // checked for acls
        if (resident.getCalcMediUPR1() && OPDE.getAppInfo().isAllowedTo(InternalClassACL.UPDATE, internalClassID)) {
...
//            list.add(printDaily);
    ...
    private void buildPanel() {
        ...
        for (Prescription prescription : lstPrescriptions) {
            log.trace(prescription.getID() + ".xprescription");
