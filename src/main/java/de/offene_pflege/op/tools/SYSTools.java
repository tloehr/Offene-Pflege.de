...
//    }
    ...
    /**
     * see: http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4380536 Puh, das hier ist aus der Sun Bug Datenbank.
     * Etwas krude... Ich hoffe die lassen sich mal was besseres einfallen.
     */
...
//        return rid;
    ...
    /**
     * Berechnet aus einer Datei die MD5 Signatur. Teilweise auf http://rgagnon.com/javadetails/java-0596.html
     *
     * @param file
     * @return
     * @throws Exception
...
//    }
    ...
    public static void addAllNodes(DefaultMutableTreeNode root, List children) {
        if (children.size() > 0) {
    ...
    /**
     * Ermittelt die Zeichendarstellung eines Objekts (toString). Ist das Ergebnis null oder eine leere Zeichenkette,
    ...
    /**
     * Gibt die toString Ausgabe eines Objektes zurück. Hierbei kann man sicher sein, dass man nicht über ein
     * <code>null</code> stolpert.
     *
    ...
    public static String getWindowTitle(String moduleName) {
        ...
        return OPDE.getAppInfo().getProgname() + moduleName + ", " + OPDE.getAppInfo().getVersion();
    }
...
//    }
    ...
    /**
     * Einfache Funktion, die einen Text mit einer HTML Dokumentenstruktur umschließt und dadurch auch den Titel des
     * Browsers setzt. Auf Wunsch wird auch direkt ein javascript hinzugefügt, dass die Browser Druckfunktion startet.
     *
...
//        Color selectionBackground = new Color(57,105,138);  // Nimbus Background Selection Color
    ...
    /**
     * Setzt eine Split Pane (animiert oder nicht animiert) auf eine entsprechende Position (Prozentual zwischen 0 und
     * 1)
     *
     * @param split
     * @param stop
    ...
    public static String getThrowableAsHTML(Throwable exc) {
        ...
        if (OPDE.getMainframe().getCurrentResident() != null) {
            html += SYSConst.html_h3("ResID: " + OPDE.getMainframe().getCurrentResident().getId());
        }
    ...
    /**
     * @param filePath name of file to open. The file can reside anywhere in the classpath
    ...
    /**
     * Seit JDK11 musste diese Funktion überarbeitet werden, weil die Beträge nicht mehr richtig geparsed wurden.
     *
     * @param test
     * @return
     */
    public static BigDecimal parseCurrency(String test) {
        ...
        if (num != null) {
            if (num instanceof Long) {
                ...
                betrag = new BigDecimal(num.doubleValue()).setScale(2, RoundingMode.HALF_UP);
     ...
     * @param one
     * @param two
     * @return 1 if one != null && two == null, -1 if one == null && two != null, 0 if one and two are both == null or
     * both != null.
     */
