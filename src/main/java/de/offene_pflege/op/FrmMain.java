...
import de.offene_pflege.op.allowance.PnlAllowance;
import de.offene_pflege.op.care.PnlCare;
import de.offene_pflege.op.care.med.structure.PnlMed;
import de.offene_pflege.op.care.supervisor.PnlHandover;
import de.offene_pflege.op.controlling.PnlControlling;
import de.offene_pflege.op.dev.PnlDev;
import de.offene_pflege.op.process.PnlProcess;
import de.offene_pflege.op.settings.PnlSettings;
import de.offene_pflege.op.system.DlgLogin;
...
public class FrmMain extends JFrame {
    ...
    private Map<Integer, Set<Resident>> markierung_fuer_bewohner;
    ...
    public void removeBesonderheit(ResInfoType type, final Resident resident) {
        if (!type.isAlertType() && type.getType() != ResInfoTypeTools.TYPE_ABSENCE) {
            return;
        }
        synchronized (markierung_fuer_bewohner) {
            markierung_fuer_bewohner.get(type.getType()).remove(resident);
        }
        synchronized (iconPanels) {
            setIconPanel(iconPanels.get(resident), resident);
        }
    ...
    public void addBesonderheit(ResInfoType type, final Resident resident) {
        if (!type.isAlertType() && type.getType() != ResInfoTypeTools.TYPE_ABSENCE) {
            return;
        }
        ...
        synchronized (markierung_fuer_bewohner) {
            markierung_fuer_bewohner.get(type.getType()).add(resident);
        }
        synchronized (iconPanels) {
            setIconPanel(iconPanels.get(resident), resident);
        }
...
//        logger.debug(e.getComponent().getSize().width + "x" + e.getComponent().getSize().height);
    ...
    public void afterLogin() {
        ...
        if (markierung_fuer_bewohner != null) {
            synchronized (markierung_fuer_bewohner) {
                SYSTools.clear(markierung_fuer_bewohner);
            }
        }

        markierung_fuer_bewohner = Collections.synchronizedMap(new HashMap<>());
        synchronized (markierung_fuer_bewohner) {
            markierung_fuer_bewohner.put(ResInfoTypeTools.TYPE_ABSENCE, new HashSet<>());
            markierung_fuer_bewohner.put(ResInfoTypeTools.TYPE_INFECTION, new HashSet<>());
            markierung_fuer_bewohner.put(ResInfoTypeTools.TYPE_WARNING, new HashSet<>());
            markierung_fuer_bewohner.put(ResInfoTypeTools.TYPE_ALLERGY, new HashSet<>());
            markierung_fuer_bewohner.put(ResInfoTypeTools.TYPE_DIABETES, new HashSet<>());
...
//        currentClassname = classname;
        if (classname.equals("op.allowance.PnlAllowance")) {
            ...
            panel = new PnlDev();
        } else if (classname.equals("de.offene_pflege.gui.qdvs")) {
            panel = new QDVS_Panel(jspSearch);
        }
        return panel;
    }
...
//    }
    ...
    private CollapsiblePane addNursingRecords(final Station station) {
        ...
        for (final Resident resident : residentList) {
            ...
            String titel = ResidentTools.getTextCompact(resident);
    ...
    private void setIconPanel(JPanel pnl, Resident resident) {
        pnl.removeAll();
        synchronized (markierung_fuer_bewohner) {
            if (markierung_fuer_bewohner.get(ResInfoTypeTools.TYPE_ABSENCE).contains(resident)) {
                pnl.add(new JLabel(SYSConst.icon16residentAbsent));
            }
            if (markierung_fuer_bewohner.get(ResInfoTypeTools.TYPE_INFECTION).contains(resident)) {
                pnl.add(new JLabel(SYSConst.icon16biohazard));
            }
            if (markierung_fuer_bewohner.get(ResInfoTypeTools.TYPE_DIABETES).contains(resident)) {
                pnl.add(new JLabel(SYSConst.icon16diabetes));
            }
            if (markierung_fuer_bewohner.get(ResInfoTypeTools.TYPE_ALLERGY).contains(resident)) {
                pnl.add(new JLabel(SYSConst.icon16allergy));
            }
            if (markierung_fuer_bewohner.get(ResInfoTypeTools.TYPE_WARNING).contains(resident)) {
                pnl.add(new JLabel(SYSConst.icon16warning));
            }
            if (markierung_fuer_bewohner.get(ResInfoTypeTools.TYPE_FALLRISK).contains(resident)) {
                pnl.add(new JLabel(SYSConst.findIcon(SYSConst.icon16falling)));
            }
            if (markierung_fuer_bewohner.get(TYPE_BESONDERHEIT_KURZZEITPFLEGE).contains(resident)) {
                pnl.add(new JLabel(SYSConst.findIcon(SYSConst.icon16kzp)));
            }
        }
    }
...
//    }
    ...
    private void cleanup() {
        ...
        synchronized (markierung_fuer_bewohner) {
            markierung_fuer_bewohner.clear();
        }
        synchronized (iconPanels) {
            iconPanels.clear();
        }
    ...
    /**
     * this class is only used for resident names that are so long, that the icons are not visible anymore. it simply
     * tells Swing, that this particular scroll pane is 16 pixels wider than it really is. dirty trick, but works.
     */
