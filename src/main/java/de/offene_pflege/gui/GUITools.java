    ...
    public static MouseAdapter getHyperlinkStyleMouseAdapter() {
        return new MouseAdapter() {
            ...
            public void mouseEntered(MouseEvent mouseEvent) {
                text = SYSTools.removeTags(((AbstractButton) mouseEvent.getSource()).getText(), "html");
                ((AbstractButton) mouseEvent.getSource()).setText("<html><u>" + text + "</u></html>");
            }

            @Override
            public void mouseExited(MouseEvent mouseEvent) {
                ((AbstractButton) mouseEvent.getSource()).setText(SYSTools.toHTMLForScreen(text));
            }
        };
    }
...
//    }
    ...
    /**
     * Shows a JidePopup in relation to its owner. Calculates the new position that it leaves the owner visible. The
     * popup is placed according to the <code>location</code> setting. The size of the content pane is taken into the
     * calculation in order to find the necessary <code>x, y</code> coordinates on the screen.
     * <p>
     * <ul>
...
//        log.debug(Boolean.toString(isFullyVisibleOnScreen(popup, desiredPosition)));
    ...
    private static Point getDesiredPosition(JidePopup popup, int location) {
        ...
        // todo: warum ?
        // workaround. beim zweiten mal klicken (bei PnlEditResinfo) werden die owners gelöscht. warum ?
        if (popup.getOwner() == null) return centerOnScreen(popup);
...
//        GraphicsDevice gd = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice();
    ...
    public static JPanel getDropPanel(FileDrop.Listener dropListener, String text) {
        ...
        JLabel dropLabel = new JLabel(text, SYSConst.icon48kgetdock, SwingConstants.CENTER);
    ...
    public static boolean checkMod(int modifiers, int mask) {
